...
class LaTeXROCReaderAveGenerator(LaTeXROCReaderGenerator):
    def __init__(self):
        super().__init__()
        self.__reader_files = []
        self.__readerave_colors = ['blue', 'red']
        
        self.__readerave_color_definitions = ""
        self.__readerave_plot_frame_commands = ""
        self.__plot_readerave_fig_commands = ""
        self.__plot_readerave_group_commands = ""
        self.__plot_readerave_fig_commands = ""
    ...
    def parse_readerave_files(self, ave_files, reader_files, ave_names=None, type_names=None, group_names=None):
        ...
        """
        self.__parse_ave_files(ave_files, ave_names=ave_names)
        self.__parse_reader_files(reader_files, type_names=type_names, group_names=group_names)
        
        self.__readerave_color_definitions = self.__generate_readerave_color_definitions()
        self._LaTeXROCAveGenerator__ave_plot_commands = self._LaTeXROCAveGenerator__generate_ave_plot_commands()
        self._LaTeXROCReaderGenerator__reader_plot_commands = self._LaTeXROCReaderGenerator__generate_reader_plot_commands()
        self._LaTeXROCReaderGenerator__make_figure_command = self._LaTeXROCReaderGenerator__generate_make_figure_command()
        self.__readerave_plot_frame_commands = self.__generate_readerave_plot_frame_commands()
        self.__plot_readerave_group_commands = self.__generate_readerave_group_commands()
        self.__plot_readerave_fig_commands = self.__generate_plot_readerave_fig_commands()
        self._LaTeXROCReaderGenerator__header_footer = self._LaTeXROCReaderGenerator__generate_header_footer()

    def __parse_ave_files(self, group_files, ave_names=None, engine='openpyxl'):
        """
        ...
        for group_file, group_name in zip(group_files, ave_names):
            ...
            group_data = [(group_file, self.__clean_name(sheet)) for sheet in sheets]
            groups.append((group_name, group_data))
            
        self._LaTeXROCAveGenerator__ave_groups = groups        
        self._LaTeXROCAveGenerator__ave_index_map = self._LaTeXROCAveGenerator__generate_ave_index_map()
        self._LaTeXROCAveGenerator__num_ave_colors = max(len(group_data) for _, group_data in self._LaTeXROCAveGenerator__ave_groups)
        self._LaTeXROCAveGenerator__ave_colors = self._LaTeXROCReaderGenerator__default_colors[:self._LaTeXROCAveGenerator__num_ave_colors]
        
        self._LaTeXROCAveGenerator__ave_data_commands = self._LaTeXROCAveGenerator__generate_ave_data_commands()

    def __parse_reader_files(self, reader_files, type_names=None, group_names=None, engine='openpyxl'):
        """
        Parses the reader ROC Excel files and prepares the data for LaTeX commands.
        """
        groups = []
        type_names = type_names or list(self._LaTeXROCReaderGenerator__generate_names(len(reader_files)))
        
        if group_names is None:
            group_names = [list(self._LaTeXROCReaderGenerator__generate_names(len(type_files))) for type_files in reader_files]
        ...
        for type_files, type_name, methods in zip(reader_files, type_names, group_names):
            ...
            for file, method_name in zip(type_files, methods):
                ...
                self._LaTeXROCReaderGenerator__sheet_names.update(sheets)
                method_sheets.append((method_name, file, sheets))
            groups.append((type_name, method_sheets))
        
        self.__reader_files = reader_files
        self._LaTeXROCReaderGenerator__reader_groups = groups
        self._LaTeXROCReaderGenerator__reader_index_map = self._LaTeXROCReaderGenerator__generate_reader_index_map()

        max_files_per_type = max(len(methods) for _, methods in self._LaTeXROCReaderGenerator__reader_groups)
        self._LaTeXROCReaderGenerator__reader_colors = [self._LaTeXROCReaderGenerator__default_colors[:max_files_per_type]] * len(type_names)
        self._LaTeXROCReaderGenerator__reader_marks = [self._LaTeXROCReaderGenerator__default_styles[:max_files_per_type]] * len(type_names)

        self._LaTeXROCReaderGenerator__reader_data_commands = self._LaTeXROCReaderGenerator__generate_reader_data_commands()

    def __generate_readerave_color_definitions(self):
        """
        Generates LaTeX commands to define colors for average and reader ROC curves.
        """
        color_definitions = "% Define ROC curve colors:\n"
        color_definitions += f"""
\\definecolor{{COLORo0}}{{named}}{{{self.__readerave_colors[0]}}}
\\definecolor{{COLORo1}}{{named}}{{{self.__readerave_colors[1]}}}
...
"""
    ...
    def generate_readerave_plot_frame_commands(self):
        ...
        for ave_group_index, (ave_group_name, ave_group_data) in enumerate(self._LaTeXROCAveGenerator__ave_groups):
            for ave_file_index, (ave_file, ave_sheet_name) in enumerate(ave_group_data):
                ave_cmd_index = f"{ave_group_name}_{ave_sheet_name}"
                ave_letter_index = self._LaTeXROCAveGenerator__ave_index_map[ave_cmd_index]
                plot_frame_commands += f"\\newcommand{{\\PlotFRAMEoReadernAve{ave_letter_index}}}{{\n"

                for type_index, (type_name, methods) in enumerate(self._LaTeXROCReaderGenerator__reader_groups):
                    for method_index, (method_name, file, sheets) in enumerate(methods):       
                        if (ave_group_index == type_index and ave_file_index == method_index):                                        
                            for sheet_index, sheet_name in enumerate(sheets):
                                ...
                                reader_letter_index = self._LaTeXROCReaderGenerator__reader_index_map[reader_cmd_index]
    ...
    def __generate_readerave_group_commands(self):
        """
...
"""
    ...
    def __generate_plot_readerave_fig_commands(self):
        """
...
"""
    ...
    def generate_latex_document(self):
        ...
        """
        latex_document = (
            self._LaTeXROCReaderGenerator__generate_document_header() +
            self._LaTeXROCAveGenerator__ave_data_commands +
            self._LaTeXROCReaderGenerator__reader_data_commands +

            self.__readerave_color_definitions +
            self._LaTeXROCAveGenerator__ave_plot_commands +
            self._LaTeXROCReaderGenerator__reader_plot_commands +
            
            self.__readerave_plot_frame_commands +
            self.__plot_readerave_fig_commands +
            self.__plot_readerave_group_commands +
            self._LaTeXROCReaderGenerator__make_figure_command +
            
            self._LaTeXROCReaderGenerator__header_footer +
    ...
    def generate_latex_image(self):
        ...
        """
        latex_document = (
            self._LaTeXROCReaderGenerator__generate_image_header() +
            self._LaTeXROCAveGenerator__ave_data_commands +
            self._LaTeXROCReaderGenerator__reader_data_commands +
    ...
    def export_readerave_settings(self):
        ...
        """
        settings = {
            'page_format': self._LaTeXROCReaderGenerator__page_format,
            'plot_format': self._LaTeXROCReaderGenerator__plot_format,
            'readerave_colors': self.__readerave_colors
        }
        return settings
    ...
    def import_readerave_settings(self, settings):
        ...
        self._LaTeXROCReaderGenerator__page_format = settings['page_format']
        self._LaTeXROCReaderGenerator__plot_format = settings['plot_format']
        self.__readerave_colors = settings['readerave_colors']
            
        self._LaTeXROCReaderGenerator__document_header = self._LaTeXROCReaderGenerator__generate_document_header()
        self._LaTeXROCReaderGenerator__make_figure_command = self._LaTeXROCReaderGenerator__generate_make_figure_command()
        self.__readerave_color_definitions = self.__generate_readerave_color_definitions()
